...
%% Get the pairs to be annotated
mask = zeros(N,N);
b = floor(N/L); %blocklength
for i=1:L
	...
	for itr = 1:maxitr   
		...
		for ii=1:length(pairs)
			...
			if(B_t(i,j)==G_all(i,j))
				c=binornd(1,p_err);
				if(c==0)
					G_all_new(i,j)=G_all(i,j);
				else
					G_all_new(i,j)=double(~G_all(i,j));
				end
			end
		end
